...
export function StarredPage() {
  ...
  const { boards, loading, error, fetchStarredBoards, starBoard, unstarBoard } =
    useStarredBoards();
  ...
  return (
    <div className="p-6">
      <PageTitle title={t("dashboard.starred")} />
      ...
      {loading ? (
        ...
        <div className="rounded-[var(--radius-md)] overflow-visible">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((board) => (
                <tr key={board.id} className="hover:bg-[var(--bg-tertiary)]">
                  <td className="px-4 py-3">
                    <Link
                      to={`/boards/${board.id}`}
                  ...
                  <td className="px-2 py-3">
                    <BoardActions
                      starred
                      t={t}
                      onStar={() => starBoard(board.id)}
                      onUnstar={() => unstarBoard(board.id)}
                      onCopyLink={() =>
          ...
          footer={
            <>
              ...
              <Button
                ...
                onClick={() => handleDelete(deleteBoardId)}
              >
